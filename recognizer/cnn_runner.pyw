    ...
    def apply_pattern_match(self):
        ...
        for i in range(0, len(self.data)):
            ...
            if results[0] == 1:
                ...
                list_err.append(Error(int(top_left[0] + w / 2), int(top_left[1] + h / 2), err_name[len(err_name) - 1]))
        ...
        if err == True:
            ...
            for i in range(0, len(list_err)):
                cv2.line(colorful_image, (list_err[i].x - CNNRunner.CONST_WIDTH_LINE, list_err[i].y), (list_err[i].x + CNNRunner.CONST_WIDTH_LINE, list_err[i].y), (0, 0, 255), 3)
                cv2.line(colorful_image, (list_err[i].x, list_err[i].y - CNNRunner.CONST_WIDTH_LINE), (list_err[i].x, list_err[i].y + CNNRunner.CONST_WIDTH_LINE), (0, 0, 255), 3)
                cv2.circle(colorful_image,(list_err[i].x, list_err[i].y), int(CNNRunner.CONST_WIDTH_LINE / 2), (0,0,255), -1)
                font = cv2.FONT_HERSHEY_SIMPLEX
                cv2.putText(colorful_image, list_err[i].err_name, (int(list_err[i].x + CNNRunner.CONST_WIDTH_LINE / 2), int(list_err[i].y - CNNRunner.CONST_WIDTH_LINE / 2)), font, 1, (0, 0, 255), 2, cv2.LINE_AA)
